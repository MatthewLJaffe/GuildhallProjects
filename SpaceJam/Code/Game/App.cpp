...
void App::StartUp()
{
	...
	windowConfig.m_windowTitle = "Space Jam";
...
void App::HandleSpecialCommands()
{
	...
	if (g_theInput->WasKeyJustPressed('T'))
	{
		...
		{
			g_theAudio->SetSoundPlaybackSpeed(g_theGame->m_gameScene->GetTrackLayerPlaybackID((TrackLayerType)i), .25f * BEAT_SPEED_MULT);
		}
	}
	if (g_theInput->WasKeyJustReleased('T'))
	{
		...
		for (int i = 0; i < (int)TrackLayerType::COUNT; i++)
		{
			g_theAudio->SetSoundPlaybackSpeed(g_theGame->m_gameScene->GetTrackLayerPlaybackID((TrackLayerType)i), BEAT_SPEED_MULT);
		}
	}
...
{
	g_theGame->ShutDown();
	ModelDefinition::RemoveModelDefinitions();
