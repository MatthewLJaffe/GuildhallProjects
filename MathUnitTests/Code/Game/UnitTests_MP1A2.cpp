...
//------------------------------------------------------------------------------------------------
void CountFloatRngResult( float randomFloat, int arraySize, int numBuckets, int* resultsArray, float minLegal, float maxLegal )
{
	if( randomFloat >= minLegal && randomFloat <= maxLegal )
	{
		// Result in legal range; increment count in array bucket
		float fractionInRange = (randomFloat - minLegal) / (maxLegal - minLegal);
		int resultBucket = int( fractionInRange * float(numBuckets) );
		resultBucket = Clamp( resultBucket, 0, numBuckets-1 );
		++ resultsArray[ resultBucket ];
	}
	else
	{
		++ resultsArray[ arraySize-1 ]; // N-1 spot in array is reserved for illegal "out of bounds" count
		...
		if( timesSeenThisMessage < 3 )
		{
			printf( "\n  ERROR: An RNG function returned random float %f - outside legal range [%g,%g]!", randomFloat, minLegal, maxLegal );
			++ timesSeenThisMessage;
		}
	}
}
...
void VerifyAndReportIntRngTest( int arraySize, int* resultsArray, int minLegal, int maxLegal, int numRolls )
{
	...
	VerifyTestResult( illegalResultCount == 0, "RNG function returned one or more illegal (out-of-range) ints!" );
...
//------------------------------------------------------------------------------------------------
void VerifyAndReportFloatRngTest( int arraySize, int numBuckets, int* resultsArray, float minLegal, float maxLegal, int numRolls )
{
	float validRange = maxLegal - minLegal;
	float rangePerBucket = validRange / float(numBuckets);
	int averageCountPerResult = numRolls / numBuckets;
	int maxCountDeviation = averageCountPerResult / 5; // Allow up to +/- 20% variation on expectation
	int illegalResultCount = resultsArray[ arraySize-1 ];

	// Check if there were any illegal (out-of-range) results, e.g. returning 3 or 9 if we want a number in [4,7]
	VerifyTestResult( illegalResultCount == 0, "RNG returned one or more illegal (out-of-range) floats!" );
	if( illegalResultCount > 0 )
	{
		float illegalResultPercent = 100.f * static_cast< float >( illegalResultCount ) / static_cast< float >( numRolls );
		printf( "\n  Out of %i rolls, %i (%.0f%%) were outside the legal range [%g,%g] the RNG should have provided", numRolls, illegalResultCount, illegalResultPercent, minLegal, maxLegal );
	}
	...
	if( worstCountDeviation > maxCountDeviation )
	{
		int worstCount = resultsArray[ worstCountDeviationIndex ];
		float worstDeviationPercent = 100.f * static_cast< float >( worstCountDeviation ) / static_cast< float >( averageCountPerResult );
		float worstCountDeviationRangeMin = (rangePerBucket * float(worstCountDeviationIndex)) + minLegal;
		float worstCountDeviationRangeMax = worstCountDeviationRangeMin + rangePerBucket;
		printf( "\n" );
		printf( "  Expected %i rolls per possible result-range (in total range [%g,%g]);\n", averageCountPerResult, minLegal, maxLegal );
		printf( "   result sub-range [%g,%g] got %i rolls (%.0f%% deviation)", worstCountDeviationRangeMin, worstCountDeviationRangeMax, worstCount, worstDeviationPercent );
	}
}
...
void RunTestsForRandomIntLessThan( int maxNotInclusive )
{
	// Set up results counters & array
	constexpr int NUM_ROLLS = 1'000'000;
	int numBuckets = maxNotInclusive; // RandomIntLessThan(5) has 5 buckets; [0] is count for result=0, [4] is count for result=4
	int* resultCountsArray = new int[ numBuckets ];
	int numValuesTooLow = 0;
	int numValuesTooHigh = 0;
	for( int bucketIndex = 0; bucketIndex < numBuckets; ++ bucketIndex )
	{
		resultCountsArray[bucketIndex] = 0;
	}

	// Do a bunch of random rolls and accumulate counts for each possible result
	RngClass rng;
	for( int i = 0; i < NUM_ROLLS; ++ i )
	{
		int randomInt = rng.Rng_RollRandomIntLessThan( maxNotInclusive );
		if( randomInt < 0 )
		{
			++ numValuesTooLow;
			static bool hasSeenThisMessage = false;
			if( !hasSeenThisMessage )
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomIntLessThan(%i) rolled a value of %i, which is too low (minimum should be %i)", maxNotInclusive, randomInt, 0 );
			}
		}
		else if( randomInt >= maxNotInclusive )
		{
			++ numValuesTooHigh;
			static bool hasSeenThisMessage = false;
			if( !hasSeenThisMessage )
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomIntLessThan(%i) rolled a value of %i, which is too high (maximum should be %i)", maxNotInclusive, randomInt, maxNotInclusive-1 );
			}
		}
		else
		{
			++ resultCountsArray[randomInt]; // result of 0 increments counter[0]
		}
	}

	// Check if we had any out-of-range values
	bool wasInRange = (numValuesTooLow == 0) && (numValuesTooHigh == 0);
	VerifyTestResult( wasInRange, "RNG.RollRandomIntLessThan() produced illegal (out-of-range) value(s)!" );

	// Analyze results and see if they are statistically plausible (we're mainly just looking for obviously-wrong here)
	int expectedCount = NUM_ROLLS / numBuckets;
	float expectedPercent = 100.f / float(numBuckets);
	int deviationCountTolerance = expectedCount / 5; // Allow no more than +/- 20% from expectation here
	int worstDeviationCount = 0;
	int worstDeviationIndex = 0;
	for( int bucketIndex = 0; bucketIndex < numBuckets; ++ bucketIndex )
	{
		// See if this is the worst deviation-from-expected-average out of all of the buckets
		int actualCount = resultCountsArray[bucketIndex];
		int deviationCount = abs( actualCount - expectedCount );
		if( deviationCount > worstDeviationCount )
		{
			worstDeviationCount = deviationCount;
			worstDeviationIndex = bucketIndex;
		}
	}

	// Report results of even distribution test
	bool wasDistributedEvenly = true; // ...until proven guilty
	if( worstDeviationCount > deviationCountTolerance )
	{
		wasDistributedEvenly = false;
		int worstDeviationValue = worstDeviationIndex;
		int actualCount = resultCountsArray[worstDeviationIndex];
		float actualPercent = 100.f * float(actualCount) / float(NUM_ROLLS);
		printf( "\n    RNG.RollRandomIntLessThan(%i) rolled a %i for %.02f%% of test rolls; should be about %.02f%% of %i rolls tested",
			maxNotInclusive, worstDeviationValue, actualPercent, expectedPercent, NUM_ROLLS );
	}
	VerifyTestResult( wasDistributedEvenly, "RNG.RollRandomIntLessThan() produced skewed (unevenly distributed) value(s)!" );

	delete[] resultCountsArray;
}
#endif

//------------------------------------------------------------------------------------------------
#if defined( ENABLE_TestSet_MP1A2_RandomNumberGenerator )
void RunTestsForRandomIntInRange( int minInclusive, int maxInclusive )
{
	// Set up results counters & array
	constexpr int NUM_ROLLS = 1'000'000;
	int numBuckets = 1 + maxInclusive - minInclusive; // RandomIntInRange(6,8) has 3 buckets; [0] is count for result=6, [2] is count for result=8
	int* resultCountsArray = new int[ numBuckets ];
	int numValuesTooLow = 0;
	int numValuesTooHigh = 0;
	for( int bucketIndex = 0; bucketIndex < numBuckets; ++ bucketIndex )
	...
	for( int i = 0; i < NUM_ROLLS; ++ i )
	{
		int randomInt = rng.Rng_RollRandomIntInRange( minInclusive, maxInclusive );
		if( randomInt < minInclusive )
		{
			++ numValuesTooLow;
			static bool hasSeenThisMessage = false;
			if( !hasSeenThisMessage )
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomIntInRange(%i,%i) rolled a value of %i, which is too low (minimum should be %i)", minInclusive, maxInclusive, randomInt, minInclusive );
			}
		}
		else if( randomInt > maxInclusive )
		{
			++ numValuesTooHigh;
			static bool hasSeenThisMessage = false;
			if( !hasSeenThisMessage )
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomIntInRange(%i,%i) rolled a value of %i, which is too high (maximum should be %i)", minInclusive, maxInclusive, randomInt, maxInclusive );
			}
		}
		else
		{
			int bucketIndex = randomInt - minInclusive;
			++ resultCountsArray[bucketIndex]; // result of min+3 increments counter[3]
		}
	}

	// Check if we had any out-of-range values
	bool wasInRange = (numValuesTooLow == 0) && (numValuesTooHigh == 0);
	VerifyTestResult( wasInRange, "RNG.RollRandomIntInRange() produced illegal (out-of-range) value(s)!" );
	...
	{
		wasDistributedEvenly = false;
		int worstDeviationValue = worstDeviationIndex + minInclusive;
		int actualCount = resultCountsArray[worstDeviationIndex];
		float actualPercent = 100.f * float(actualCount) / float(NUM_ROLLS);
		printf( "\n    RNG.RollRandomIntInRange(%i,%i) rolled a %i for %.02f%% of test rolls; should be about %.02f%% of %i rolls tested",
			minInclusive, maxInclusive, worstDeviationValue, actualPercent, expectedPercent, NUM_ROLLS );
	}
	VerifyTestResult( wasDistributedEvenly, "RNG.RollRandomIntInRange() produced skewed (unevenly distributed) value(s)!" );
...
//------------------------------------------------------------------------------------------------
#if defined( ENABLE_TestSet_MP1A2_RandomNumberGenerator )
void RunTestsForRandomFloatZeroToOne()
{
	float minInclusive = 0.f;
	float maxInclusive = 1.f;

	// Set up results counters & array
	constexpr int NUM_ROLLS = 1'000'000;
	int numBuckets = 10; // Each bucket covers 1/10 of the total range
	...
	for( int i = 0; i < NUM_ROLLS; ++ i )
	{
		float randomFloat = rng.Rng_RollRandomFloatZeroToOne();
		if( randomFloat < minInclusive )
		{
			++ numValuesTooLow;
			static bool hasSeenThisMessage = false;
			if( !hasSeenThisMessage )
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomFloatZeroToOne() rolled a value of %.03f, which is too low (minimum should be %g)", randomFloat, minInclusive );
			}
		}
		else if( randomFloat > maxInclusive )
		{
			++ numValuesTooHigh;
			static bool hasSeenThisMessage = false;
			if( !hasSeenThisMessage )
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomFloatZeroToOne() rolled a value of %.03f, which is too high (maximum should be %g)", randomFloat, maxInclusive );
			}
		}
		else
		{
			// Calculate which bucket covers the sub-range this random float value fell into
			float fractionInRange = (randomFloat - minInclusive) / range;
			int bucketIndex = int( fractionInRange * float(numBuckets) );
	...
	// Check if we had any out-of-range values
	bool wasInRange = (numValuesTooLow == 0) && (numValuesTooHigh == 0);
	VerifyTestResult( wasInRange, "RNG.RollRandomFloatZeroToOne() produced illegal (out-of-range) value(s)!" );
	...
	if( worstDeviationCount > deviationCountTolerance )
	{
		...
		int actualCount = resultCountsArray[worstDeviationIndex];
		float actualPercent = 100.f * float(actualCount) / float(NUM_ROLLS);
		float subRangePerBucket = range / float(numBuckets);
		float subRangeMin = minInclusive + (float(worstDeviationIndex) * subRangePerBucket);
		float subRangeMax = subRangeMin + subRangePerBucket;
		printf( "\n    RNG.RollRandomFloatZeroToOne() rolled a number in subrange [%g,%g) for %.02f%% of test rolls (should be about %.02f%%)",
			subRangeMin, subRangeMax, actualPercent, expectedPercent );
	}
	VerifyTestResult( wasDistributedEvenly, "RNG.RollRandomFloatZeroToOne() produced skewed (unevenly distributed) value(s)!" );
...
//------------------------------------------------------------------------------------------------
#if defined( ENABLE_TestSet_MP1A2_RandomNumberGenerator )
void RunTestsForRandomFloatInRange( float minInclusive, float maxInclusive )
{
	// Set up results counters & array
	constexpr int NUM_ROLLS = 1'000'000;
	int numBuckets = 10; // Each bucket covers 1/10 of the total range
	...
	for( int i = 0; i < NUM_ROLLS; ++ i )
	{
		float randomFloat = rng.Rng_RollRandomFloatInRange( minInclusive, maxInclusive );
		if( randomFloat < minInclusive )
		{
			++ numValuesTooLow;
			static bool hasSeenThisMessage = false;
			...
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomFloatInRange(%g,%g) rolled a value of %.03f, which is too low (minimum should be %g)", minInclusive, maxInclusive, randomFloat, minInclusive );
			}
		}
		else if( randomFloat > maxInclusive )
		{
			...
			{
				hasSeenThisMessage = true;
				printf( "\n    RandomFloatInRange(%g,%g) rolled a value of %.03f, which is too high (maximum should be %g)", minInclusive, maxInclusive, randomFloat, maxInclusive );
			}
		}
		else
		{
	...
	// Check if we had any out-of-range values
	bool wasInRange = (numValuesTooLow == 0) && (numValuesTooHigh == 0);
	VerifyTestResult( wasInRange, "RNG.RollRandomFloatInRange() produced illegal (out-of-range) value(s)!" );
	...
	if( worstDeviationCount > deviationCountTolerance )
	{
		...
		float subRangeMin = minInclusive + (float(worstDeviationIndex) * subRangePerBucket);
		float subRangeMax = subRangeMin + subRangePerBucket;
		printf( "\n    RNG.RollRandomFloatInRange(%g,%g) rolled a number in subrange [%g,%g) for %.02f%% of test rolls (should be about %.02f%%)",
			minInclusive, maxInclusive, subRangeMin, subRangeMax, actualPercent, expectedPercent );
	}
	VerifyTestResult( wasDistributedEvenly, "RNG.RollRandomFloatInRange() produced skewed (unevenly distributed) value(s)!" );
...
{
#if defined( ENABLE_TestSet_MP1A2_RandomNumberGenerator )
	RunTestsForRandomIntLessThan( 7 );
	RunTestsForRandomIntLessThan( 300 );
	RunTestsForRandomIntInRange( 2, 5 );
	RunTestsForRandomIntInRange( 6, 8 );
	RunTestsForRandomIntInRange( -9, -7 );
	RunTestsForRandomIntInRange( -2, 3 );
	RunTestsForRandomFloatZeroToOne();
	RunTestsForRandomFloatInRange( 2.f, 5.f );
	RunTestsForRandomFloatInRange( 6.5f, 8.5f );
	RunTestsForRandomFloatInRange( -9.f, -7.f );
	RunTestsForRandomFloatInRange( -2.f, 3.f );
#endif
...
void RunTests_MP1A2()
{
	printf( "Running tests for MP1-A2 (version 1)...\n" );
