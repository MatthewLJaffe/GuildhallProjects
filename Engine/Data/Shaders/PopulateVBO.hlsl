...
void CSMain( uint3 DTid : SV_DispatchThreadID )
{
	...
	ParticleEmitterUpdateDefinitionGPU config= updateDefs[emitterInstance.definitionIndex];

    //add to vbo as billboarded quad
    ...
    float3 totalVelocityWorldSpace = particle.velocity + particle.lifetimeVelocity;
    if (dot(particle.reflectionVector, particle.reflectionVector) > .01f)
    {
        totalVelocityWorldSpace = reflect(totalVelocityWorldSpace, particle.reflectionVector);
        totalVelocityWorldSpace *= .15f;
    }
    if (config.worldSimulation == 0)
    {
        totalVelocityWorldSpace = (float3)mul( emitterInstance.localToWorldMatrix, float4(totalVelocityWorldSpace, 0.f) );
    }
    ...
    {
        if (length(totalVelocityWorldSpace) > .01)
        {
            float3 velocityDir = normalize( totalVelocityWorldSpace );
    ...
    {
        float theta = GetValueInFloatGraph(floatGraphs, FLOATGRAPH_ROTATION_1D,
        emitterInstance.definitionIndex, normalizedLifePos, particleIndex + SEED_OFFSET_ROTATION_1D);
    ...
    float uniformScale = emitterInstance.localToWorldMatrix._m33;
    size *= uniformScale;
    ...
    */
    ...
    if (config.stretchBillboard == 0)
    {
        ...
        //left
        else if (config.stretchMode == 1)
        {
            bottomLeft = particleCenterPos - size.x * fullFacingJBasis - .5f * size.y * fullFacingKBasis;
            bottomRight = particleCenterPos - .5f * size.y * fullFacingKBasis;
            topRight = particleCenterPos + .5f * size.y * fullFacingKBasis;
            topLeft = particleCenterPos - size.x * fullFacingJBasis + .5f * size.y * fullFacingKBasis;
        }
        //right
        else if (config.stretchMode == 2)
        {
            bottomLeft = particleCenterPos - .5f * size.y * fullFacingKBasis;
            bottomRight = particleCenterPos + size.x * fullFacingJBasis - .5f * size.y * fullFacingKBasis;
            topRight = particleCenterPos + size.x * fullFacingJBasis + .5f * size.y * fullFacingKBasis;
            topLeft = particleCenterPos + .5f * size.y * fullFacingKBasis;
        }
        //up
        else if (config.stretchMode == 3)
        {
            bottomLeft = particleCenterPos - .5f * size.x * fullFacingJBasis;
            bottomRight = particleCenterPos + .5f * size.x * fullFacingJBasis;
            topRight = particleCenterPos + .5f * size.x * fullFacingJBasis + size.y * fullFacingKBasis;
            topLeft = particleCenterPos - .5f * size.x * fullFacingJBasis + size.y * fullFacingKBasis;
        }
        //down
        else
        {
            bottomLeft = particleCenterPos - .5f * size.x * fullFacingJBasis - size.y * fullFacingKBasis;
            bottomRight = particleCenterPos + .5f * size.x * fullFacingJBasis - size.y * fullFacingKBasis;
            topRight = particleCenterPos + .5f * size.x * fullFacingJBasis;
            topLeft = particleCenterPos - .5f * size.x * fullFacingJBasis;
        }
    }
    else
    {
        float kBasisAlignment = dot(fullFacingKBasis, totalVelocityWorldSpace);
        float jBasisAlignment = dot(fullFacingJBasis, totalVelocityWorldSpace);
