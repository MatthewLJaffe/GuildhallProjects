...
//------------------------------------------------------------------------------------------------
void AddVertsForGlyphTriangles2D( std::vector<Vertex_PCU>& m_localVerts, char glyph, const Vec2& cellMins, const Vec2& pixelSize, const Rgba8& color )
{
	...
	for( int rowIndex = 0; rowIndex < TRITEXT_PIX_HIGH; ++ rowIndex )
	{
		...
		for( int triTextCellIndex = 0; triTextCellIndex < TRITEXT_PIX_WIDE; ++ triTextCellIndex )
		{
			...
			SimpleTriangleFont_AddVertsForAABB2D( m_localVerts, AABB2( minX,minY, maxX,maxY ), color, Vec2(0.f,0.f), Vec2(1.f,1.f) );
		}
	}
};
...
//------------------------------------------------------------------------------------------------
void AddVertsForTextTriangles2D( std::vector<Vertex_PCU>& m_localVerts, const std::string& text, const Vec2& startMins, float cellHeight, const Rgba8& color, float cellAspect, bool isFlipped, float spacingFraction )
{
...
//	verts.reserve( verts.size() + estimatedNumberOfNewVerts );
	...
	{
		char glyph = text[i];
		AddVertsForGlyphTriangles2D( m_localVerts, glyph, cellMins, pixelSize, color );
