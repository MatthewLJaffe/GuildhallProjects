...
StructuredBuffer<MeshEmissionVert> meshEmissionVerts : register(t2);
...
int GetEmitterConfigIndex(uint DTidx)
{
	uint beginIndex = 0;
...
void CSMain( uint3 DTid : SV_DispatchThreadID )
{
	...
	int instanceIdx = GetEmitterConfigIndex(DTid.x);
	ParticleEmitterInstanceGPU emitterInstance = emitterInstances[instanceIdx];
	...
	Particle particle;
	particle.configIndex = instanceIdx;
	particle.position = float3(0.f, 0.f, 0.f);
	//SPHERE
	if (config.emissionType == 0)
	{
	...
	else if (config.emissionType == 2)
	{
		...
		MeshEmissionVert v1 = meshEmissionVerts[meshToEmitFrom.offset + triangleToEmitFrom * 3 + 0];
		MeshEmissionVert v2 = meshEmissionVerts[meshToEmitFrom.offset + triangleToEmitFrom * 3 + 1];
		MeshEmissionVert v3 = meshEmissionVerts[meshToEmitFrom.offset + triangleToEmitFrom * 3 + 2];
