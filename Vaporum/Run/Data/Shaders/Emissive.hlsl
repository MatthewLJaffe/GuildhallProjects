...
Texture2D gTex0 : register(t5);

Texture2D GetTextureFromGlobalArray(int textureIndex)
{
	if (textureIndex == 0)
	{
		return gTex0;
...
ps_output_t PixelMain(v2p_t input)
{
	...
	//panning texture
	if (renderDef.textureToPanIndex >= 0 && dot(color.rgb, color.rgb) > 0.01f)
	{
		float2 panTextureUV = input.quadUV;
		...
		float4 panTextureColor = GetTextureFromGlobalArray(renderDef.textureToPanIndex).Sample(diffuseSampler, panTextureUV);
	...
	//emissive render target cannot write to alpha channel or things get weird...
	output.emissiveRenderTarget = float4(input.emissive * color.rgb, 0.f);
	return output;
}
