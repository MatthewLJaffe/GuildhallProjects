...
//StructuredBuffer<ParticleEmitterUpdateDefinitionGPU> emitterConfigs : register(t0);
//StructuredBuffer<ParticlePhysicsObjectGPU> physicsObjects : register(t1);
...
StructuredBuffer<ParticlePhysicsObjectGPU> physicsObjects : register(t2);
StructuredBuffer<FloatGraph> floatGraphs : register(t3);
StructuredBuffer<Float2Graph> float2Graphs : register(t4);
StructuredBuffer<Float3Graph> float3Graphs : register(t5);
...
RWStructuredBuffer<float> particleDistanceBuffer : register(u5);
RWStructuredBuffer<uint> particleDrawList : register(u6);
...
void CSMain( uint3 DTid : SV_DispatchThreadID )
{
    ...
    if (config.returnToOriginForce > 0.f)
    {
        ...
        if (particle.emissionPosition.x != particle.position.x || particle.emissionPosition.y != particle.position.y || particle.emissionPosition.z != particle.position.z)
        {
            //delay return
	...
	if (config.worldSimulation == 1)
    {
        particle.position += mul(emitterInstance.localToWorldMatrix, float4(totalVelocity, 0.f)).xyz * c_deltaSeconds;
    }
    else
    {
    ...
    //check if particle is in frustum view
    ...
    size *= abs(emitterInstance.localToWorldMatrix._m00);
