...
{
	if (m_renderDebug && m_debugVertexes.size() > 0)
	{
		RenderDebug();
	}
	if (m_unlit)
...
void Prop::RenderDebug() const
{
	g_theRenderer->SetModelConstants(GetModelMatrix(), Rgba8::WHITE);
	g_theRenderer->SetDepthMode(DepthMode::ENABLED);
	g_theRenderer->SetBlendMode(BlendMode::OPAQUE);
	g_theRenderer->SetRasterizeMode(RasterizeMode::SOLID_CULL_BACK);
	g_theRenderer->BindTexture(nullptr);
	g_theRenderer->BindShader(nullptr);
	g_theRenderer->DrawVertexBuffer(m_debugVertexBuffer, (int)m_debugVertexes.size(), 0, VertexType::VERTEX_TYPE_PCU);
}
...
void Prop::CreateQuad(Vec2 const& size)
{
	m_unlitVertexes.clear();
	m_debugVertexes.clear();
	m_vertexes.clear();
	...
	//m_unlitVertexes.push_back(Vertex_PCU(Vec3(.5f * size.x, -.5f * size.y, 0.f), m_color, Vec2(1.f, 0.f)));
	//m_unlitVertexes.push_back(Vertex_PCU(Vec3(.5f * size.x, .5f * size.y, 0.f), m_color, Vec2(1.f, 1.f)));
	//m_unlitVertexes.push_back(Vertex_PCU(Vec3(-.5f * size.x, -.5f * size.y, 0.f), m_color, Vec2::ZERO));
	
	//m_unlitVertexes.push_back(Vertex_PCU(Vec3(-.5f * size.x, -.5f * size.y, 0.f), m_color, Vec2::ZERO));
	//m_unlitVertexes.push_back(Vertex_PCU(Vec3(.5f * size.x, .5f * size.y, 0.f), m_color, Vec2(1.f, 1.f)));
	//m_unlitVertexes.push_back(Vertex_PCU(Vec3(-.5f * size.x, .5f * size.y, .0f), m_color, Vec2(0.f, 1.f)));

	m_vertexes.push_back(Vertex_PCUTBN(Vec3(-.5f * size.x, -.5f * size.y, 0.f), m_color, Vec2::ZERO, Vec3(1.f, 0.f, 0.f), Vec3(0.f, 1.f, 0.f), Vec3(0.f, 0.f, 1.f)));
	m_vertexes.push_back(Vertex_PCUTBN(Vec3(.5f * size.x, -.5f * size.y, 0.f), m_color, Vec2(1.f, 0.f), Vec3(1.f, 0.f, 0.f), Vec3(0.f, 1.f, 0.f), Vec3(0.f, 0.f, 1.f)));
	m_vertexes.push_back(Vertex_PCUTBN(Vec3(.5f * size.x, .5f * size.y, 0.f), m_color, Vec2(1.f, 1.f), Vec3(1.f, 0.f, 0.f), Vec3(0.f, 1.f, 0.f), Vec3(0.f, 0.f, 1.f)));
	m_vertexes.push_back(Vertex_PCUTBN(Vec3(-.5f * size.x, .5f * size.y, .0f), m_color, Vec2(0.f, 1.f), Vec3(1.f, 0.f, 0.f), Vec3(0.f, 1.f, 0.f), Vec3(0.f, 0.f, 1.f)));
...
void Prop::CreateHexGrid(MapDefinition const& mapDef)
{
	...
	m_unlitVertexes.clear();
	AABB3 worldBounds = AABB3(mapDef.m_worldBoundsMin, mapDef.m_worldBoundsMax);
	m_unlit = true;
	for (int y = 0; y < mapDef.m_gridSize.y; y++)
	{
		for (int x = 0; x < mapDef.m_gridSize.x; x++)
		{
			Vec2 hexPos = GetHexPosFromGridCoord(IntVec2(x, y));
			if (worldBounds.IsPointInside(Vec3(hexPos.x, hexPos.y, 0.f)))
			{
				TileDefinition tileDef = g_theGame->GetTileDefinitionBySymbol(mapDef.m_tiles[x + mapDef.m_gridSize.x * y]);
				if (tileDef.m_isBlocked)
				{
					g_theGame->AddVertsForHexCell(m_unlitVertexes, IntVec2(x, y), Rgba8::BLACK, false);
				}
				else
				{
					g_theGame->AddVertsForHexCell(m_unlitVertexes, IntVec2(x, y), Rgba8::WHITE, true);
				}
			}
		}
	}
